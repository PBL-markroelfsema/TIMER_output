...
{ library(tidyverse)
  ...
  if (as.character(Variable_graph) %in% names(Scenario_i1) & as.character(Variable_graph) %in% names(Scenario_i2))
  {  #tmp_NoPolicy <- filter(NoPolicyi[[as.character(Variables_in_sceny[i,3])]], region==as.character(Variables_in_sceny[i,2]), year >= StartYear, year <= EndYear) %>% select(year, region, value, unit)
    ...
    g<- ggplot() + 
      ...
      scale_colour_discrete(labels=c(deparse(substitute(Scenario_i1)), deparse(substitute(Scenario_i2)))) +
      ylim(0,NA) + 
