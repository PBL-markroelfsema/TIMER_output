...
add_variables <- function(all,scens){
    ...
    source("functions/factsheet/calcVariable.R")
    source("functions/factsheet/calcRel2Base.R")
    ...
    # 1. REN share in demand sector (excl electricity)
    # 2. REN share region x electricity in demand sector
    # 3. add 1 and 2
...
#    all <- calcVariable(all,'`Policy Cost` ~ `Policy Cost|Other` ' , newUnit='billion US$2010/yr')
    ...
    source("functions/factsheet/calcBudget.R")
...
#    all <- overwrite(remind::calcCumulatedDiscount(all, discount = 0.05, nameVar = "GDP|MER"), all)
    ...
    source("functions/factsheet/calcRel2BaseYear.R")
