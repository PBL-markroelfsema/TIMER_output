...
name_Appliances="Appliances and light"
...
Scenarios <- c("Current policies", "Good practice policies", "2-degree scenario (2.6 W/m2) with 66% probability")
...
# 7. lulucf sector
LULUCF <- filter(GPP_Deforestation_data, year>=2015, year<=2030, indicator=="CO2 emissions LULUCF")
...
data_fig1a_1 <- rbind(CPS_fig1a_1, GPP_fig1a_1) %>% rbind(TwoC_fig1a_1)
...
fig1a_left <- ggplot() +
  ...
  scale_fill_manual("reductions", values = c("#f7fbff", "#deebf7", "#c6dbef", "#9ecae1", "#6baed6", 
                                             "#4292c6", "#2171b5", "#08519c", "#08306b", "white")) +
  scale_colour_manual("scenario", values = c("black","black", "darkolivegreen4")) + # colour lines 
...
ggsave(file=paste("graphs/figure1.png",sep=""),fig1a,width=20,height=12,dpi=200)
...
#ResFin <- GPPPolicies_indicators$Residential_FinalEnergy_m2
#ResFin$value <- ResFin$value*10^-3
#Residential_FinalEnergy_m2 <- rbind(CurrentPolicies_indicators$Residential_FinalEnergy_m2, ResFin)
